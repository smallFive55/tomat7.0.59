...
public class Catalina {
    ...
    protected Digester createStartDigester() {
        ...
        // 对xml文件中的Server节点创建一个StandardServer对象
        digester.addObjectCreate("Server",
                                 "org.apache.catalina.core.StandardServer",
                                 "className");
        //根据Server节点中的属性信息调用相应属性的setter方法，调用setPort、setShutdown方法，入参分别是8005、SHUTDOWN
        ...
        // 对xml的Server节点下的Listener节点，取className属性的值作为实例化类实例化一个对象
        digester.addObjectCreate("Server/Listener",
                                 null, // MUST be specified in the element
                                 "className");
        digester.addSetProperties("Server/Listener");
        // 调用StandardServer.addLifecycleListener，注册监听器
    ...
    public void load() {
        ...
        try {
            // 调用server【StandardServer】的init()方法
            getServer().init();
        } catch (LifecycleException e) {
    ...
    public void start() {
        ...
        // Start the new server
        try {
            // 调用server【StandardServer】的start()方法
            getServer().start();
        } catch (LifecycleException e) {
