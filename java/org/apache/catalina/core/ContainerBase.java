...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // 向容器发布一个 START_EVENT 事件
        setState(LifecycleState.STARTING);

        // Start our thread
        // 启动线程 ContainerBackgroundProcessor
        threadStart();
    ...
    public void backgroundProcess() {
        ...
        if (!getState().isAvailable())
            return;
        // 逐个调用与容器相关组件 Cluster、Loader、Manager、Realm、Valve 的 backgroundProcess() 方法
        Cluster cluster = getClusterInternal();
        if (cluster != null) {
            try {
    ...
    protected void threadStart() {
        // 当启动 StandardEngine 时，初始化 ContainerBackgroundProcessor 线程【backgroundProcessorDelay 参数在 StandardEngine 初始化时设置】
        if (thread != null)
            return;
