...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req,
        ...
        // Request与Response对象转化从org.apache.coyote转为org.apache.catalina.connector
        // Tomcat中请求响应处理真实操作对象为org.apache.catalina.connector.Request/Response
        Request request = (Request) req.getNote(ADAPTER_NOTES);
        Response response = (Response) res.getNote(ADAPTER_NOTES);
        ...
        try {
            ...
            // request parameters
            req.getRequestProcessor().setWorkerThreadName(Thread.currentThread().getName());
            // 调用请求对象内保存了本次请求具体要求执行的Host、Context、Wrapper 组件的引用
            postParseSuccess = postParseRequest(req, request, res, response);
            if (postParseSuccess) {
                //check valves if we support async
                request.setAsyncSupported(connector.getService().getContainer().getPipeline().isAsyncSupported());
                // Calling the container
                // 通过Container容器【StandardEngine】的Pipeline逐个调用
                // getPipeline()获取ContainerBase.pipeline【StandardPipeline】
                // StandardEngine 构造时 pipeline.setBasic(new StandardEngineValve()); 设置的Valve
                // Engine、Host、Context、Wrapper 容器组件的实现的共通点：
                //  1.这些组件内部都有一个成员变量 pipeline，因为都继承ContainerBase
                //  2.都是在类的构造方法中设置管道内的基础阀。
                //  3.所有的基础阀的实现最后都会调用其下一级容器的 getPipeline().getFirst().invoke() 方法，直到 Wrapper 组件。
                //    因为 Wrapper 是对一个 Servlet 的包装，所以它的基础阀内部调用的过滤器链的 doFilter 方法和 Servlet 的 service 方法。
                connector.getService().getContainer().getPipeline().getFirst().invoke(request, response);
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req,
        ...
        while (mapRequired) {
            ...
            // 通过 map 方法的调用设置 request 的成员变量 mappingData 的成员变量 host、context、warp 信息
